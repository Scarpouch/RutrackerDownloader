...
class RutrackerDownloader:
    ...
    def setup_logging(self):
        """Создает логгер с ежедневным ротационным файлом"""
        self.log_dir = "logs"
        ...
        self.logger.error("🧪 Тестовая ОШИБКА записана")
        self.logger.debug("🧪 Тест DEBUG записан")
    ...
    def load_config(self):
        ...
        if os.path.exists(config_file):
            ...
            # ✅ ИСПРАВЛЕНО: Правильное создание дефолтного конфига
    ...
    def save_config(self):
        try:
            ...
            if self.folder_path:
                ...
                self.config.set('paths', 'last_folder', '')
            
            # ✅ ИСПРАВЛЕНО: используем self.config
            with open('downloader.ini', 'w', encoding='utf-8') as f:
                self.config.write(f)
            
            self.log(f"💾 Конфиг сохранен: {self.root.winfo_width()}x{self.root.winfo_height()}")
    ...
    def create_gui(self):
        ...
        # ✅ Загрузка последней папки (один раз)
        ...
        # ✅ Автосохранение при изменении размера окна
    ...
    def on_window_resize(self, event):
        """Автосохранение конфига при изменении размера окна"""
    ...
    def check_file_exists(self):
        ...
        try:
            ...
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=15,
                                  cwd=self.folder_path, creationflags=0x08000000,
                                  startupinfo=startupinfo)
            filename = result.stdout.strip()
            if filename:
                full_path = os.path.join(self.folder_path, filename)
                if os.path.exists(full_path):
                    self.log(f"⚠️ Файл уже существует: {full_path}")
